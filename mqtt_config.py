...
if MQTT_TOPIC_STYLE == "path":
    ...
    DEFAULT_TOPIC_CHAT = f"{MQTT_TOPIC_NAMESPACE}/+/{MQTT_PROJECT_SLUG}/chat"
    DEFAULT_TOPIC_CHAT_RESPONSE = f"{MQTT_TOPIC_NAMESPACE}/{{device_id}}/{MQTT_PROJECT_SLUG}/chat-result"
else:
    DEFAULT_TOPIC_TELEMETRY = f"{MQTT_TOPIC_NAMESPACE}/+/{MQTT_PROJECT_SLUG}-telemetry"
    DEFAULT_TOPIC_RESPONSE = f"{MQTT_TOPIC_NAMESPACE}/{{device_id}}/{MQTT_PROJECT_SLUG}-result"
    DEFAULT_TOPIC_FEEDBACK = f"{MQTT_TOPIC_NAMESPACE}/{{device_id}}/{MQTT_PROJECT_SLUG}-feedback"
...
TOPIC_CHAT = os.getenv("TOPIC_CHAT", DEFAULT_TOPIC_CHAT).strip()
TOPIC_CHAT_RESPONSE = os.getenv("TOPIC_CHAT_RESPONSE", DEFAULT_TOPIC_CHAT_RESPONSE).strip()
...
MQTT_TLS_ENABLED = _get_bool("MQTT_TLS_ENABLED", False)
MQTT_TLS_CA_CERT = _get_str("MQTT_TLS_CA_CERT", "")
MQTT_TLS_INSECURE = _get_bool("MQTT_TLS_INSECURE", False)
...
# =========================
# MQTT Rate Limit Configuration
# =========================
# Mencegah satu device/topic membanjiri queue RAG/LLM.
# Set 0 atau angka negatif untuk mematikan rate limit.
MQTT_MAX_MESSAGES_PER_MINUTE = _get_int("MQTT_MAX_MESSAGES_PER_MINUTE", 6)
MQTT_CHAT_MAX_MESSAGES_PER_MINUTE = _get_int("MQTT_CHAT_MAX_MESSAGES_PER_MINUTE", 6)
MQTT_RATE_LIMIT_SCOPE = _get_str("MQTT_RATE_LIMIT_SCOPE", "device").lower()
MQTT_PROCESSING_TIMEOUT_SECONDS = _get_int("MQTT_PROCESSING_TIMEOUT_SECONDS", 90)
MQTT_QUEUE_WARN_SIZE = _get_int("MQTT_QUEUE_WARN_SIZE", max(10, int(QUEUE_MAXSIZE * 0.7)))
MQTT_QUEUE_LOG_INTERVAL_SECONDS = _get_int("MQTT_QUEUE_LOG_INTERVAL_SECONDS", 30)
MQTT_MAX_PAYLOAD_BYTES = _get_int("MQTT_MAX_PAYLOAD_BYTES", 16384)
MQTT_CHAT_MAX_QUESTION_CHARS = _get_int("MQTT_CHAT_MAX_QUESTION_CHARS", 1000)
MQTT_DUPLICATE_WINDOW_SECONDS = _get_int("MQTT_DUPLICATE_WINDOW_SECONDS", 10)
